 ...
 *     class B extends A
 *     
 *     A a=new B() => 이렇게 만드는것을 동적 바인딩 이라고 한다
 *         -------
 *         A가 가지고 있는 메소드 => 오버라이딩된 메소드 호출
 *                     ==== 주소     
 *     - A가 가지고 있는 변수 : 이처럼 일반변수는 주소가 없다 그래서 바꿀 수 없다
 *      => 정적 바인딩 => C언어가 더 빠른 이유다.
 *      서버는 C 이고 어플은 java를 쓴다.
 *     
 */
import javax.swing.*;
import java.awt.*;
