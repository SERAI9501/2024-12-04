...
public class RecipeMain {
	 ...
	 * 
	 * ArrayList<ChefVO> list=new ArrayList<ChefVO>(); DataDAO
	 * dao=DataDAO.newInstance(); try { int k=1; for(int i=1;i<=44;i++) { //
	 * https://www.10000recipe.com/chef/chef_list.html?order=chef_no_follower&term=
	 * all&page=2 Document doc=Jsoup.connect(
	 * "http://www.10000recipe.com/chef/chef_list.html?order=chef_no_follower&term=all&page="
	 * +i).get(); Elements poster=doc.select("div.list_mem3 a.mem_pic img");
	 * Elements chef=doc.select("div.list_cont4 a"); Elements
	 * mem_cont1=doc.select("span.mem_cont1"); Elements
	 * mem_cont3=doc.select("span.mem_cont3"); Elements
	 * mem_cont7=doc.select("span.mem_cont7"); Elements
	 * mem_cont2=doc.select("span.mem_cont2");
	 * 
	 * for(int j=0;j<poster.size();j++) { try { ChefVO vo=new ChefVO();
	 * vo.setPoster(poster.get(j).attr("src")); vo.setChef(chef.get(j).text());
	 ...
	 * System.out.println("k="+k); System.out.println(
	 * "---------------------------------------------------------");
	 * dao.chefInsert(vo); k++; }catch(Exception ex){} //list.add(vo); } }
