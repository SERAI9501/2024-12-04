 ...
 *  데이터 저장
 *   = 변수 : 한개 데이터 저장
 *   = 배열 : 여러개를 연속적으로 저장 => 인덱스 : 1.고정적 2.같은 데이터형 만 저장가능 
 *   = 클래스 : 여러개 데이터 저장 => 다른 데이터형 => 동적 (new)
 *   ------------------------------------------------ 메모리에 저장
 *   = 파일 => 종속(파일과 파일 의 연결이 어렵다)
 *   = DBMS(관계형데이터베이스=오라클)
 *  사용자가 요청하면 => 처리
 *   =연산자
 *   =제어문
 ...
 *             ----------  ------------------
 *             1개만 가능          제한이 없다
 *                              =>매게변수를 많이 사용하면 ex) 3개 이상이라면?
 *                                 =>묶어서 설정 (클래스,배열로 묶으라는 말)
 *                              
 *             => 자바/C=> 1개만 사용
 *             return 값
 *              여러개값이 있는 경우 : 배열/클래스로 넘기면 된다.
 *             => 파이썬 => 여러개 가능[데이터형 섞는것도 가능]
 *             return 값,값,값 이런식으로 가능하다
 *             변수,변수,변수=메소드 이런식으로 가능하다
 *             
 *             => 프로그램
 *             	기본
 *                 = 입력기능
 *                 = 요청처리 기능 => 세분화
 *                 = 출력기능
 *                 = 모든 메소드는 한개의 기능만 수행 => 이렇게 해야 유지보수가 쉽다.
 *                   => 결제창 => 등록
 *                 = 교통시스템
 *                     사고 / 공사 => 출력
 *                     CCTV => 위도 / 경도만 알면 OPENCV로 cctv를 가져 올 수 있다.
 *                          --- 최적화(속도,사용자 편리성)
 *               =>프로그램 => 개발보다 유지보수에 초점 
 *                                  ----- 가독성
 *                                  메소드 변경 / 메소드 추가
 ...
 *     --------------------------------------------------
 *               X                     X
 *               void System.out.println()  (HTML은 오른쪽 의미와 같다.) <br>
 *               ----                  --
 *               다음출에 출력한다
 *     --------------------------------------------------
 *     
 *     메소드 호출
 *     ---------
 *      메소드명(전송하는 값) => void일때 쓰는방식
 *      데이터형 변수=메소드명(전송하는 값)
 *              |     |
 *              -------
 *      => 왼쪽에 있는 데이터형이 크거나 같다 => 권장(같은 데이터형
 *      => 통계 / 회계는 큰 데이터를 설정하는게 편함
 *      
 *     메소드 제작
 *     1)반복이 많은 경우
 *     2)기능별로 나눠서 처리 => 단락
