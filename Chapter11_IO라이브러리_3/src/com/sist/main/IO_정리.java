 ...
 *  1) 특징
 *    = 단방향 => Input / Output (빨대)
 *     => 읽기/쓰기 => 쓰레드 이용해서 처리하면 읽기쓰기가 동시에 가능
 *     => 네트워크 에는 전송(쓰기) / 수신(읽기) 가  있다
 *                  -------   -------- 
 ...
 *       => ObjectInputStream / ObjctOutputStream
 *     = ~Reader / ~Writer
 ...
 *      w => ~OutputStream/~Writer => Create(덮어씌워진다)
...
//        	String msg=in.readLine();// enter를 누르면 메모리에 저장해준다
