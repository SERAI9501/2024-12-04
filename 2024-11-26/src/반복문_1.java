//68page
/*  웹개발 ==> 데이터베이스 (SQL)
 *  = server단 : BackEnd => 자바 / 스프링 / 데이터베이스 연동
 *  = db단 : DBA
 *  = view단 : Front => 자바스크립트
 *                      Jquery => VueJS => React => Next
 *                      ------ 4,5버전 새로나옴
 *          (옵션사항)MSA / DevOps / AWS[최근유행]
 *                 --- Spring Cloud
 ...
 *         {
 *           조건이 true일때 수행 => false 면 건너뛴다
 *         }
 *         => 오류처리 => 8장 예외처리
 *         => 비정상 종료를를 방지하고 프로그램을 유지
 *         =>if 문은 유효성검사를 가장 많이 짠다.
 *       선택조건문 => true 일때 false일때 처리를 나눠서 처리
 *         if(조건문)
 *         {
 *          조건이 true 일때 처리
 *         }
 ...
 *  선택문 : switch~case
 *         다중 조건과 유사
 *         =======범위를 지정할 수 있다는 장점이 있다., 값을 한개만 선택 ex 게임,어플
 *         형식)
