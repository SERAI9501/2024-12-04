 ...
 *              }catch(Exception e){}
 *             }
 *            //크롤링의 경우 for문 안에 try를 걸어야 한다. 필요 없는부분은 스킵하고 움직이게끔
 *            // HTML => 모든 HTML이 형식이 동일하지 않기 때문에 위 같이 해야한다.
 *            
 *            =>중요 오라클은 commit => 저장X
 *            => finally 안에도 try catch안에도 try를 사용이 가능하다 데이터 연동할때 자주 사용한다.
 *            => 트렌젝션 => 일괄처리(*********) 
 *            
 *            => 공통모듈 만들기 = 반복되는 과정을 모아서 한번에(하나의 메소드로) 처리하도록 만드는것
 *            	
 *              
 *    2. 간접처리 (예외회피) => 선언만 한다
 *       메소드뒤에 throws
 *       => 직접처리하는 부분이 아니고 이런 에러가 예상이 된다고
 *          컴파일러에게 알려준다
 ...
 *                             ----------------------
 *                             구분은 ,(컴마)로 하고 순서는 상관없다
 *          => 처리시에는 통합
 *                     --- Exception / Throwable
 *                     --- catch 절 , throws 절=> 통합
 *       => 임의로 발생 : throw => 사용자 정의 예외처리
 *       => 모든게 똑같지만 => class, method, 예외처리, 데이터형
 *                        ----------------------------
 *                        => 완벽하게 지원하지 않기에
 *                           ----------------- 사용자 정의가 있다. =너희에 맞게 만들어서 사용해라
 *         
 */
