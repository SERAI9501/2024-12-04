...
public class ReplyBoardDAO {
	...
	public void replyInsert(int pno,ReplyBoardVO vo)
	{
		try {
			...
			// SQL => 4개
			String sql="SELECT group_id,gruop_step,group_tab "
					+ "FROM replyBoard"
					+ "WHERE no="+pno;
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
			...
			//2.SQL => group step을 변경 => 답변 핵심
			sql="UPDATE replyBoard SET "
					+ "group_setp=group_step+1 "
			...
			ps.executeUpdate();
			conn.commit();
			// JSP에서는 MyBatis로 코딩한다
		} catch (Exception ex) {
			//트랜잭션을 일괄처리 => 스프링에서는 1파트
			try {
	...
	public boolean replyDelete(int no,String pwd)
	{
		...
		try {
			...
			conn.setAutoCommit(false);
			// SQL
			String sql="SELECT pwd,root,dqpth "
					+ "FROM replyBoard "
					+ "WHERE no="+no;
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeUpdate();
			rs.next();
			String db_pwdString=rs.getString(1);
