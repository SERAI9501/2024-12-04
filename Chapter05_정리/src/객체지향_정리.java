 ...
 *              class A
 *              {
 *              	A(int a){}// 생성자 추가가 안된다. = 매개변수 있는것도 생성자이다.
 *              }
 *              class A
 *              {
 *              	// 생성자 추가
 ...
 *          => Math.random() => 이건 메모리가 이미 저장되어있기때문에 객체를 생성하지 않는다.
 *     }
 *  2. 멤버변수
 *  ----------------------------------------------------------------
 ...
 *  4. 메소드
 *  5. 접근지정어 
 *     클래스 , 메소드 , 멤버변수 / 지역변수에서는 사용이 불가능
 *                           ------ final
 *     private : 자신의 클래스에서만 사용이 가능
 *               은닉화 => 모든 멤버변수는 private로 설정 하게 될 것이다.
 *     public : 모든 클래스에서 사용이 가능
 *              => 클래스 , 메소드 , 생성자
 *     protected : 같은 패키지안에서 사용이 가능
 *                 다른 패키지 => 상속받은 클래스
 *                 => 사용빈도가 거의 없다
 *     default : 같은 패키지에서만 사용이 가능
 *               윈도우 => 버튼 , 편집기
 *               
 *     ** public : 클래스는 필요시 모든 클래스에 사용이 가능
 *                 생성자 , 메소드
 *     ** private : 멤버변수 => 은닉화
 *     ** 접근 범위
 ...
 *                  => 생성자 첫번째 위치
 *     static{}를 사용하는 방법 => static변수로 배열/클래스 => 초기화시에 주로 사용
 *                                       --- ----라이브러리 (MyBatis)
 *                                       목록
