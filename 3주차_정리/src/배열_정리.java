 ...
 *                  2) int이하 => char,short,byte=>연산이 되면 결과값이 int 다.
 *                     int + long = long
 *                     char+char = int
 *                     byte+byte = int
 *                    + 산술(더하기)
 ...
 *           단일조건문=>조건이 true일 경우에만 처리
              ...
              {
                     조건이  true 일때 처리 문장
                     로그인시 =< 브라우저에 전송(main/ㅎ[ㅇ;ㅈ; ;ㅇ;ㄷㅇ
              }
              }
              else
              {
                     조건이 false일때 처리 문장
                     로그인 실패시 => 다시 로그인, 회원가입 등              
              }
 *        2ㅣ반복문
 *           for :   
 *            반복횟수가 지정
 ...
 *              => 한개 이름으로  제어
 *              => 변수
 *              => 클래스 여러개 (한개 이르이름으로(발견))
 *          예)
 *           Borad => 글쓰기 , 목록 , 상세보기 , 수정 ,찾기 , 예약하기 , 좋아요
 *           ----- 클래스
 *           Food 목록(페이징) , 상세보기 ,찜하기 ,예약하기 ,좋아요
 *           Recipe => 목록 , 쉐프정보 , 레시피 제작 , 구독 ,검색
 *           seoul => 목록 , 여행코스 , 인근 맛집 , 숙소 , 랜트키대여
 *       배열 (일차배열)
 *        = 선언
 *          데이터형[] 배열명;=>권장
 *          데이터형 배열명[];=> C언어 호환
 ...
 *              ArrayList / Map / Set 배열대신 나중에 이런것들을 사용한다.
 *              
 *             **int [] arr=new int[5];
 *                         arr[0]
 *                            -- 인덱스 / 첨자 => 데이터 구분자
 ...
 *                            void 메소드명()
 *                            ---- 결과값이 없는 경우
 *                            System.out,println()=값을 받지 않기에 void에 해당한다
 *                    메소드명 : 변수 식별자
 *                             알파벳,한글 시작
 *                             알파벳은 대소문자 구분
 *                             숫자는 사용 => 앞에서 사용 금지
 ...
 *                                 약간의 의미만 부여
 *                             *** 7~15
 *                    매개변수 : 여러개 사용이 가능 , 없는 경우도 있다 예)Math.random(),trim()
 ...
 *                5) return 사용법
 *                   ------ 메소드가 종료의미
 *                   리턴형이 있는 경우에반드시 return 값;
 *                   리턴형이 없는 경우엔 return을 생략 가능
 *                    => 자동으로 컴파일러에 의해 자동 생성/즉 모든 메소드는 return이 있다는 의미
 *                    *** 반드시 리턴을 필요하단 것.
 *                    
 ...
 *                mvnrepository.com<=위 사이트에 들어가 있다.
 *                
 *              => 객체지향 프로그램 : Actor를 알아야 한다.
 *              => 클래스의 구성 요소
 *                 = 변수
 *                   변수의 종류
 *                 = 생성자
