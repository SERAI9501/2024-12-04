 ...
 *             데이터를 오라클 전송 => INSERT
 *             오라클 닫기 => static
 *        *** 프로그램의 핵심 : 반복 제거 => 반복적인 부분을 메소드화 시키면 편해진다. => public
 *        
...
class Util
{
	 ...
	 *      => 윈도우
	 */
	public int[] getRand(int count)//Call By V
	{
		int[] com=new int[count];
		for(int i=0;i<com.length;i++)
		{
			com[i]=(int)(Math.random()*31)+1;
			for(int j=0;j<i;j++) // j는 저장된 데이터를 가져오는 것
			{
				if(com[i]==com[j])
				{
					i--;
	...
	public void getRand(int[] com)//Call By Reference
	{
	for(int i=0;i<com.length;i++)
	{
		com[i]=(int)(Math.random()*31)+1;
	...
	public void display()
	{
		System.out.println("display()진입2");
		System.out.println("display()기능 처리3");
		System.out.println("display() 종료4");
	}
	// 정렬 => asc/desc
...
//        	System.out.print(i+" ");
//        }
//		System.out.println("main에서 display 호출 전1");
//		util.display();
//		System.out.println("main에서 display 호출 완료5");
	}
