 ...
 *        예)
 *           컴퓨터
 *             CPU / 하드디스크 / 모니터 / 그래픽 카드 / 메모리 ...
 *             ------------------------------------------
 *             | 메인보드 (조립)
 *             사이트
 *               회원 관련 , 예약 관련 , 구매(장바구니) , 커뮤니티 
 *               , 영화 관련
 *               -------------------------------------
 *               메인페이지에서 연결 => 메뉴
 *         객체지향 프로그램 특징
 ...
 *          => 상속 : 중복제거 => 메모리를 효율적으로 사용한다.
 *                   단점) 속도가 느려진다 / 영향력이 크다 => 스프링에서는 사용 빈도가 거의 없다.
 *        1) 용어
 *           객체
 *           클래스
 *           멤버변수
 ...
 *         ---------new 생성자()
 *         => 멤버변수 : Heap에 저장 => 다른 데이터형을 모아서 저장
 *         메모리 공간을 한개만 만들어서 공유 : 공유 변수 = > static
 *                                    -------
 *                                    영화목록 / 음악목록 / 회원관리
 *                                    -------
 *                                    오라클(static) => 공유할 목적
