 ...
 *             객체가 null / 클래스 형변환 ...
 *             
 *          1) 사전에 차단 (우선시 => if) => 안될때 예외처리
 *          2) 에러 와 예외처리
 *             = 에러 => 소스상에서 수정이 불가능한 에러
 *                      메모리가 부족 , 이클립스 ...
 *                      치명적인 에러 => 처리가 불가능
 *             = 예외처리 => 가벼운 에러 = 소스상에서 수정이 가능한 에러
 ...
 *          => 쓰레드
 *      MalformedURLEception
 *          => URL
 *          
 *       => java.io / java.sql / java.net 이친구들이 나오면 예외처리를 해야한다.
 ...
 *                  => 웹 / 윈도우 => 정수입력...
 *                  => " 10" => 정수형 변경이 안된다 = 오류나는 코드 => trim()
 *                  => 웹 => board_lost.jsp?page = 10 => 오류 = 띄어쓰기 오류
 *             => RuntimeException
 *                       |
 ...
 *                  B b=(B)a;  => 이게 정상적인 코딩
 *                  
 *                 NumberFormatException : 문자열을 정수형
 *                 
 *                 => catch 여러개인 경우
 *                    계층 순서로 적용
 *                    
