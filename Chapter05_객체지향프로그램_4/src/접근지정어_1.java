...
/*
 *  1. 주의점
 *     1) static 메소드에서는 인스턴스 변수, 인스턴스메소드를
 *       사용 할 수 없다
 *       => 사용시에는 객체 생성후에 사용이 가능
 *     2) static 메솓,. satatic변수는 this를 사용할 수 없다
 ...
 *            생성자 , 메소드
 *  ---------------------------------------------------------
 *  private  멤버변수 , 생성자       자신의 클래스 내부      클래스 외부에서는 사용이 금지
 *              메소드
 *  ---------------------------------------------------------
 *  *** 많이 사용
 *      멤버변수 _ 은닉화 private
 *      생성자, 클래스 , 메소드 =>모든 클래스 연결: : .pibliic
 */
// => 크기
//private < default < protected < public
//| 오버라이딩 접근지정어 처리
//   =>접근지정어는 확장은 가능 => 축소는 안한다
//   
//***많이 사용
//멤버변수 => 은닉화 private
/*
생성자 , 클래스 , 메소드 => 모든 클래스 연결 : 
-------------------- 모든 클래스의 연결
                       다른 폴더에 존재할 수 있다
 제어어
  =>static | abstract | final
  static => 공텅적인 =>공통으로 사용되는 메소드 / 변수
  .final => 마지막 => 변수(상수)) , 메소드(종단) , 클래스(종단)		
  								-----------------------
  								|있는 그대로 사용
  								|Strin,Math,Scanner
  abstract : 추상적인 => 공통기능 => 선언만 함
             => 추상클래스 , 인터페이스  => 구현 (X)
                ----------------- 미완성 클래스
                -- 서로 다른 클래스를 연결해서 사용
                -- 클래스 여러개 제어
...
class Member
{
	private String id;// Member클래스 안에서만 빨간색 
	protected String pwd;//같은 폴더 상에서 볼 수 있다 노란색
	int age;//같은 폴더 상에서 볼 수 있다 파랑색
	public String name;// 모든 클래스에서 접근이 가능하다 초록색
}
