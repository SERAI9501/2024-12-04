 ...
 *           ------ 메소드안 선언 , 매개변수를 지역변수라고 한다.
 *           ------ 메소드 종료되면 자동으로 메모리해제
 *           ------ 생성시점 : 메소드 호출시에 생성
 *           ---------------------------------------
 *                자동 초기화가 안된다 => 초기화 후에 사용
 *           ---------------------------------------
 *              메모리에 저장되는 시점   메모리 해제 되는 시점
 *           ---------------------------------------
 *           인스턴스   객체생성          프로그램 종료
 *                  new 생성자()      =>Heap영역에 저장 된다.
 *                                  =>사용자 영역
 *                                  =>GC
 *                                   자동 메모리 회수
 *           ---------------------------------------
 ...
 *          2) Stack : 지역변수 , 매개변수 저장
 *          3) Heap : 배열 , 인스턴스 변수...
 *          -------------------------------
 *          
 *     ---------------
 *     생성자 => 초기화블럭 => 초기화
 *     ---- 초기화 시작하는 프로그램 => 시작하는 프로그램 없으면 생략 가능
 *     ---- 반드시 클래스명과 동일해야 한다.
 *     Scanner scan=new Scanner(System.in);
 *                      -------
 *                      위 부분이 생성자
 *                      생성자는 반드시 호출시에 new라는게 필요하다
 *                      new => 클래스의 메모리 크기 확인
 *                          =>메모리 배정
 *                      생성자 => 초기화
 *                      .=>주소 접근 연산자
 ...
 *  protected => 같은 폴더에서만 접근이 가능
 *               상속받은 클래스는 다른 폴더에서도 접근이 가능
 *  default => 같은 폴더에서만 접근이 가능(************)
 *  private => 자신의 클레스에서만 접근이 가능
 *  5) 초기화 => 변수
 *  6) 멤버메소드
 *  7) 메모리 할당
 *  8) 클래스 종류
 ...
 *          	}
 *         }
 *         ***** 어떤 프로그램에서 사용하는지 사용처를 알아둬야 한다.
 *         ***** 개념 : 면접 , 사용처 (구현)
 *         게시판 , 관리자페이지 ...
 *         
 *  9) 예외처리 : 사전에 에러 방시 , 비정상 종료를 방지하고 정상 상태 유지
 *     = 예외복구 try~catch
 ...
 *        대표적으로 java.io / java.sql / java.net
 *        예외 : 수정이 가능한 에러
 *             => 소스상에서
 *             서버 => ip
 *             오라클 => 주소
 *             파일 => 파일명
 *             => 메모리 할당할 공간이 없다 => 종료해야함 => 에러
 *  ------------------ 5,6,7,8장(자바 문법)
 *  => 객체지향에 맞게 구현보다는 형식 / 구현
 *  
 *    9장 조립 : 라이브러리 ~ 12장부터
 *    getConnection() => 오라클에 연결하는 애
 *    
