 ...
 *   
 *   메모리에 저장
 *   클래스명 객체명=new 클래스명()//참조 변수=Stack 은 메모리 위치값만 알려주고 Heap이 실제 데이터다.
 *   ----- ---- ----
 *              클래스 크기에 맞게 메모리 생성 => 객체명에 생성된 주소를 넘겨준다
 *              new => 리턴형 => 메모리 주소(정수형)
 *              new => 메소드 int malloc(클래스)
 *                          ------------ 메모리 저장 => 주소리턴
 *                          => 승격 => new
 *                          => 메모리 해제 => free() => delete
 *                                                   |사용하지 않는다
 *                                                   |GC
 *          변수=>여러개 변수 제어가능 차이가 있다.
 *   데이터형 (사용자 정의)
 *   
 *   객체 활용
 *   객체명.변수 , 객체명.메소드 .....
 ...
 *   설계 ===> 저장 ==> 활용 ==> 소멸
 *   ---------------------------
 *   소멸 => GC 바로 회수하지 않는다 => 프로그램 종료시에 회수
 *   => 윈도우 / 웹 => 최적화
 *   --------------------------------------------
 *   눈에 보이거나 / 가상 => 객체화
