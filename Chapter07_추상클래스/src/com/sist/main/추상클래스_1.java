 ...
 *   abstract classs A => A a=new A(); 오류 발생
 *   => 상속을 내려서 상속받은 클래스를 미완성 된 메소드를 구현해서 사용
 *   => 사용 방법
 *   abstract class A
 *   class B extends A
 ...
 *     ---------------
 *     예시로 마우스 가 있다
 *     업체들이 많은데 운영체제에 맞게 구현 MS사에서 제공하는 추상 클래스를 구현
 *   => 데이터베이스의 모든 클래스는 추상클래스로 만들어져 있다
 *     => 개발자가 데이터베이스에 맞게 구현
 *     => DB종류 오라클,MySQL,MariaDB,DB2...
 *     
 *     => SQL문장은 표준화 (데이터형 ,함수)
 *   => 추상클래스 형식
 ...
 *      {
 *      }
 *      public abstract void aaa(); <=위 부분이 없을 수 있다.
 *      
 *   }
 *   
 *   =>단일 상속 => 나중에 다중상속
